"""US-A-03 — Create a cairn exploration."""
...
def test_us_a_03_creates_exploration_with_owner_prefix(
    cwd: Path, monkeypatch: pytest.MonkeyPatch
):
    root = _bootstrap(cwd, monkeypatch)
    res = runner.invoke(
        app, ["exploration", "start", "Try Alt Loss Function"], catch_exceptions=False
    )
...
def test_us_a_03_writes_exploration_manifest(cwd: Path, monkeypatch: pytest.MonkeyPatch):
    root = _bootstrap(cwd, monkeypatch)
    runner.invoke(app, ["exploration", "start", "try alt loss"], catch_exceptions=False)
    manifest = root / "explorations" / "kyle" / "try-alt-loss.md"
...
def test_us_a_03_updates_explorations_readme_on_main(
    cwd: Path, monkeypatch: pytest.MonkeyPatch
):
    root = _bootstrap(cwd, monkeypatch)
    runner.invoke(app, ["exploration", "start", "try alt loss"], catch_exceptions=False)
    ...
    text = repo.git.show(f"{main_branch}:explorations/README.md")
    assert "kyle/try-alt-loss" in text
    assert "kyle" in text


def test_us_a_03_user_left_on_new_git_branch(cwd: Path, monkeypatch: pytest.MonkeyPatch):
    root = _bootstrap(cwd, monkeypatch)
    runner.invoke(app, ["exploration", "start", "try alt loss"], catch_exceptions=False)
    repo = Repo(root)
...
def test_us_a_03_collision_prompts_for_resolution(
    ...
    runner.invoke(app, ["exploration", "start", "try alt loss"], catch_exceptions=False)
    # Return to main before trying to create a same-named exploration
    Repo(".").git.checkout("master" if "master" in [h.name for h in Repo(".").heads] else "main")
    res = runner.invoke(
        app, ["exploration", "start", "try alt loss"], catch_exceptions=False
    )
...
def test_us_a_03_requires_as_when_multiple_collaborators(
    ...
    res = runner.invoke(app, ["exploration", "start", "explore X"], catch_exceptions=False)
    assert res.exit_code != 0
...
def test_us_a_03_as_flag_picks_collaborator(
    ...
    runner.invoke(app, ["collaborator", "add", "--id", "maria", "--name", "M", "--role", "postdoc"])
    res = runner.invoke(
        app, ["exploration", "start", "explore X", "--as", "maria"], catch_exceptions=False
    )
    assert res.exit_code == 0, res.output
    assert (root / "explorations" / "maria" / "explore-x.md").is_file()


def test_us_a_03_unknown_as_id_errors(cwd: Path, monkeypatch: pytest.MonkeyPatch):
    _bootstrap(cwd, monkeypatch)
    res = runner.invoke(
        app, ["exploration", "start", "explore X", "--as", "ghost"], catch_exceptions=False
    )
    assert res.exit_code != 0
